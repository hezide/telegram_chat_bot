...
- Polling telegram bot: the alternative for polling is to use webhook from telegram. this is more robust and scalable but will require configuring the server in a non-local environment, routing, firewalls etc..  
  polling keeps a connection open but since we support only a single connection to a telegram client it is fine
- When multiple frontend clients are connected, they all receive the messages from Telegram and messages sent from any single web client.
